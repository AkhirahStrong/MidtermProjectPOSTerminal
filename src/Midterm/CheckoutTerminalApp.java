		...
		do {
			...
			do {
				...
				if (Products.productList().get(select).getAmountOfProduct() >= quantity) {
					...
					System.out.println("Error! We only have " + Products.productList().get(select).getAmountOfProduct()+ " of this style in stock, please select an amount in range.");
				}
			...
			System.out
					.println("Would you like to redisplay the menu and make a new order? (Press 1 for yes, 2 for no)");
